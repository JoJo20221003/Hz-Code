...
H_redshift_bin4,Hz_bin4,Hzerr_bin4 = np.loadtxt('Hzbin4.txt', unpack=True, usecols=(0, 1,2))
H_redshift_bin5,Hz_bin5,Hzerr_bin5 = np.loadtxt('Hzbin5.txt', unpack=True, usecols=(0, 1,2))
H_redshift_bin6,Hz_bin6,Hzerr_bin6 = np.loadtxt('Hzbin6_new.txt', unpack=True, usecols=(0, 1,2))
H_redshift_bin7,Hz_bin7,Hzerr_bin7 = np.loadtxt('Hzbin7_new.txt', unpack=True, usecols=(0, 1,2))

c_sys=np.loadtxt('C_sys_new.txt')
C=c_sys
C_1=np.linalg.inv(C)

bins=[0,0.0184,0.0315,0.11,0.22,0.32,0.52,2.4]
...
def lnprior(theta):
    ...
    H6=theta[5]
    H7=theta[6]
    if  50<H1<80 and 50<H2<80 and 50<H3<80 and 50<H4<80 and 50<H5<80 and 50<H6<80 and 50<H7<80  :
        return 0.0
    return -np.inf
...
if __name__ == "__main__":
    with Pool() as pool:
        canshu=[h1_true,h2_true,h3_true,h4_true,h5_true,h6_true,h7_true]
        ndim, nwalkers = 7, 32
...
#save all the data
test_sampler = sampler.chain[:,500:,:].reshape(sampler.chain.shape[0]*(sampler.chain.shape[1]-500),7)
np.savetxt('bin7_h_4000_Om0.3.txt', test_sampler, fmt='%5.6f')
